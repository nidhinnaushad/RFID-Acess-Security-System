...
String card_ID=""; // 
String password="[pswd]" ; // Windows / any Account's Password
String rfid="1872020316";// UID
void setup() {
  Serial.begin(9600); 
  SPI.begin();
  mfrc522.PCD_Init(); 
...
boolean isModifier(int keycode) {
    ...
    if (keycode >= 224 && keycode <= 231) { // if keycode is a modifier key
        result = true;
    }
    return result;
}
...
void pressModifier(int keycode) {
    int modifiermask = 0;
    if (isModifier(keycode)) { // if keycode represents a modifier key
        modifiermask = getModifierMask(keycode);
        buf[0] = buf[0] | modifiermask;
        Serial.write(buf, 8); // key report
    }
}
...
void pressKey(int keycode) { // Cycle the 6 key spots in the report buffer instead of just using buf[2] each time.
    buf[2] = keycode;
    Serial.write(buf, 8); // key report
}
...
void releaseKey(int keycode) {
    // find keycode in the report buffer, set it to zero.
    int i=0;
    for (i=2; i<8; i++) {
        if (buf[i] == keycode) {
            buf[i] = 0;
...
void pressSequenceOfKeys(const char * keySequence[], int numberOfKeys) {
    // This function is good for pressing a few keys while holding a modifier down for example.
...
int getKeycode(String keyname) {
    String key = String(keyname); // a copy so that we don't mutate the user's String. Not sure if this is needed, but just in case. TODO: find out.
    key.toLowerCase();

    int keycode = 0; // means nothing pressed.
...
int getModifierMask(int keycode) { // return value 0 means key is not a modifier.
    int modifiermask = 0;

    // NOTE: these are not the usage keycodes like for other keys, but rather bit masks.
